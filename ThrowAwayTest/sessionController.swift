...
class SessionController: NSObject, MCSessionDelegate, MCNearbyServiceBrowserDelegate, MCNearbyServiceAdvertiserDelegate {
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        ...
        case .connected:
            ...
            if ((UIDevice.current.name.compare((session.connectedPeers.first?.displayName)!) == .orderedDescending) || (UIDevice.current.name.compare((session.connectedPeers.first?.displayName)!) == .orderedSame)){
                firstUser = "\(UIDevice.current.name)"
            } else {
                firstUser = "\((session.connectedPeers.first?.displayName)!)"
            }
            
            print("Me: \(UIDevice.current.name)\nOther: \(session.connectedPeers.first?.displayName)\nFirst: \(firstUser)")
